...
\end{figure}
The accuracy of the sun was tested by comparing the results from the algorithms used in the project and by other sun position calculators and by creating a device to calculate the position of the sun at the current time. Using the solar position calculator from satellite-caculations.com\cite{solarpos} the solar altitude and azimuth at 15:19 on October 4th is 321.1353 and 41.72 degrees(including atmospheric refraction). From the sun position code used in the project the solar altitude and azimuth at 15:19 on October 4th is 321.1282367499703 and 41.698387906866145 degrees. These values are within 0.03 degrees of each other.

A rudimentary device seen in \ref{sun-contraption} was used to calculate the real physical position of the sun by finding the angle between north and the shadow shadow cast by the device and the length of the shadow created compared to the length of the device. The is made up of a piece of string tied to a weight and hung from above to ensure that the device is pointing straight downwards.
\chapter{Viewer}
\begin{figure}[h]
\centering
...
The overlay image seen in \ref{overlaytexture} is also useful when running the aggregator because it shows a 
